...
import type {
  Payment, User, PaymentProvider, DiscountCode
} from "@prisma/client";
...
// declare payment type with discount code, provider and user
export type PaymentWithDiscountProviderUser = Payment & {
    user: User;
    provider: PaymentProvider;
    discount_code: DiscountCode | null;
};

const Invoice = ({ payment }: { payment: PaymentWithDiscountProviderUser }) => {
  const {
    id,
    status,
    ref_id,
            ...
            <Table className="min-w-full">
              ...
              <tbody>
                <tr className="border-t">
                  ...
                  <td className="px-4 py-2">${Number(original_amount).toFixed(2)}</td>
                </tr>
                ...
                <tr className="border-t">
                  ...
                  <td className="px-4 py-2">${Number(vat_amount).toFixed(2)}</td>
                </tr>
                <tr className="border-t">
                  <td className="px-4 py-2">Payable Amount</td>
                  <td className="px-4 py-2">${Number(final_amount).toFixed(2)}</td>
                </tr>
              </tbody>
            </Table>
          </CardContent>
        ...
        {discount_code && (
          <Card className="">
            <CardContent className="p-4">
              ...
              <p className="">Discount Percentage: {Number(discount_code)}%</p>
              <p className="">
                Discount Amount: $
